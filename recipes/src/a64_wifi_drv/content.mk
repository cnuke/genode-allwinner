...
LIB_MK := $(addprefix lib/mk/,a64_wifi_firmware.mk vfs_a64_wifi.mk) \
          $(foreach SPEC,arm_v8,lib/mk/spec/$(SPEC)/a64_wifi.mk) \

MIRROR_FROM_REP_DIR := src/drivers/wifi/a64 \
                       src/lib/lx_emul/a64 \
                       src/include \
                       src/lib/a64_wifi \
                       $(LIB_MK)
...
PC_REP_DIR := $(GENODE_DIR)/repos/pc

MIRROR_FROM_PC_DIR := \
                      $(shell cd $(PC_REP_DIR); find src/lib/pc/lx_emul -type f) \
                      $(shell cd $(PC_REP_DIR); find src/drivers/wifi -type f) \
                      $(shell cd $(PC_REP_DIR); find src/lib/wifi -maxdepth 1 -type f) \
                      $(shell cd $(PC_REP_DIR); find src/lib/vfs/wifi -type f)
...
DDE_LINUX_LIB_MK := \
          ...
          $(foreach SPEC,arm_64,lib/mk/spec/$(SPEC)/libnl.mk) \
...
content: $(MIRROR_FROM_REP_DIR) $(MIRROR_FROM_OS_DIR) $(MIRROR_FROM_DDE_LINUX_DIR) \
         $(MIRROR_FROM_WS_PORT_DIR) $(MIRROR_FROM_LIBNL_PORT_DIR) \
         $(MIRROR_FROM_PC_DIR) cleanup-wpa
